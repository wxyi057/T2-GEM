...
def _call_model_forward(model_forward: Callable[..., Tensor], t: Tensor, x: Tensor, **kwargs) -> Tensor:
    """Adapter for ODE/SDE solvers that expect a `(t, x)` callable."""
    return model_forward(x=x, t=t, **kwargs)
...
class FlowSDE:
    ...
    def sample(
        ...
        **model_kwargs
        ...
        if model_is_cfg_ready:
            model_forward_fn = partial(model, cfg_scale=cfg_scale, uc_cond=uc_cond, cond_key=cond_key)
        else:
...
class FlowModel(nn.Module):
    def __init__(
        ...
        if isinstance(net_cfg, nn.Module):
            ...
            # Support dict-based instantiation (requires helper function)
            raise NotImplementedError(
                "Dict-based net_cfg instantiation not supported. Pass an nn.Module directly."
            )
        else:
            raise TypeError(f"net_cfg must be nn.Module or dict, got {type(net_cfg)}")
    ...
    def generate(
        ...
        reverse=False,
        return_intermediates=False,
        model_forward: Callable[..., Tensor] | None = None,
        **kwargs,
    ):
        """
        Args:
        ...
        flow_forward = model_forward if model_forward is not None else self
        ...
        if sample_kwargs.get("use_sde", False):
            results = self.sde_sampler.sample(
                init=x,
                model=flow_forward if model_forward is not None else self.net,
                model_is_cfg_ready=model_forward is not None,
        ...
        # ODE sampling
            ...
            if model_forward is None:
                ...
                ode_fn = partial(_call_model_forward, flow_forward, **kwargs, **cfg_kwargs)
            results = odeint(
                ode_fn,
                x,
                t,
